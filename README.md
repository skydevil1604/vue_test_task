...
### Порожня сторінка або `main.ts` / `404` у консолі

Якщо в **Network** або в помилці фігурує **`/src/main.ts`**, браузер отримує **девелоперський** `index.html` з гілки **`main`**, а не зібраний сайт з **`gh-pages`**.

1. **Settings → Pages:** гілка **`gh-pages`**, папка **`/`** (не `main`).
2. Примусове оновлення без кешу: **Ctrl+Shift+R** (або вікно інкогніто).
3. Перевірка, що на **`gh-pages`** лежить саме збірка: у браузері відкрийте сирий файл  
   `https://raw.githubusercontent.com/<USER>/<REPO>/gh-pages/index.html` — у `<script>` має бути шлях виду **`/<REPO>/assets/index-….js`**, а **не** `/src/main.ts`.
